...
fun main() {
  ...
  fun somaIntervaloFuncional(a: Int, b: Int): Int {
    val intervalo = a..b
    return if (a > b) 0 else intervalo.fold(0) { acc, x -> acc + x }
  }
  ...
  fun somaParesIntervaloFuncional(a: Int, b: Int): Int {
    val intervalo = a..b
    return if (a > b) 0 else intervalo.filter { it % 2 == 0 }.fold(0) { acc, x -> acc + x }
  }
  ...
  fun blackjack(pontos: Int, ases: Int): Int {
    val rangeAses = 0..ases
    val opcoes = rangeAses.map { pontos - it * 10 }
    val validas = opcoes.filter { it <= 21 }

    return if (validas.isNotEmpty()) {
        validas.max()
    } else {
        opcoes.min()
    }
  ...
  fun fibonacci(n: Int): Int {
     val rangeDeN = 1..n
     val resultado = rangeDeN.fold(Pair(0, 1)) { parAtual, _ ->
      val (termoAnterior, termoAtual) = parAtual
      val proximoTermo = termoAnterior + termoAtual
      Pair(termoAtual, proximoTermo)
    }
  ...
  fun somaFuncaoLista(lista: List<Int>, funcao: (Int) -> Int): Int {
    return lista.map(funcao).fold(0) { acc, x -> acc + x }
  }
