...
fun main() {
    ...
    fun <A> temValor(opcao: Opcao<A>): Boolean {
        ...
        // Se for Algum, retorne true; se for Nenhum, retorne false.
    }
    ...
    fun dobrarOpcao(opcao: Opcao<Int>): Opcao<Int> {
        return when (opcao) {
            is Algum -> Algum(opcao.valor * 2)
            Nenhum -> Nenhum
        }
        // Se for Algum, retorne Algum com o valor dobrado; se for Nenhum, retorne Nenhum.
    }
    ...
    fun <A> alturaArvore(arvore: Arvore<A>): Int {
        fun alturaRecursiva(arvore: Arvore<A>): Int {
            return when (arvore) {
                is Arvore.Folha -> 0
                is Arvore.No -> 1 + max(alturaRecursiva(arvore.esquerda), alturaRecursiva(arvore.direita))
            }
        }
        return alturaRecursiva(arvore)
        // Se for Folha, retorne 0; se for No, retorne 1 + o máximo entre as alturas das subárvores esquerda e direita.
    }
    ...
    fun <A> contarNosArvore(arvore: Arvore<A>): Int {
        fun contarRecursivo(arvore: Arvore<A>): Int {
            return when (arvore) {
                is Arvore.Folha -> 0
                is Arvore.No -> 1 + contarRecursivo(arvore.esquerda) + contarRecursivo(arvore.direita)
            }
        }
        return contarRecursivo(arvore)
        // Se for Folha, retorne 0; se for No, retorne 1 + o número de nós nas subárvores esquerda e direita.
    }
    ...
    fun <A> preOrdemArvore(arvore: Arvore<A>): List<A> {
        fun preOrdemRecursivo(arvore: Arvore<A>): List<A> {
            return when (arvore) {
                is Arvore.Folha -> emptyList()
                is Arvore.No -> listOf(arvore.valor) + preOrdemRecursivo(arvore.esquerda) + preOrdemRecursivo(arvore.direita)
            }
        }
        return preOrdemRecursivo(arvore)
        // Se for Folha, retorne uma lista vazia; se for No, retorne uma lista com o valor do nó seguido dos valores da subárvore esquerda e direita.
    }
