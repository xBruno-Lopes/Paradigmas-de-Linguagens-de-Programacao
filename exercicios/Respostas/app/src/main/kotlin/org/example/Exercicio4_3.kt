fun main() {
    ...
    fun ehPalindromo(palavra: String): Boolean {
        val stringToList = palavra.toList()
        var lastWord = stringToList.size - 1
        for (index in 0..<stringToList.size / 2) {
            if (stringToList[index] != stringToList[lastWord]) {
                return false
            }
            lastWord--
        }
